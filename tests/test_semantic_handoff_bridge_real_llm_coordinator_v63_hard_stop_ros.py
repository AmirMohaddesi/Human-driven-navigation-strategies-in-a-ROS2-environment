"""
V6.3 — real-adapter branch on V4.3 launch + 2-leg ``assign_named_sequence`` hard-stop smoke.

- **Default:** launch subprocess strips OpenAI env, ``use_deterministic_fake_adapter: false`` →
  handoff ``adapter_failure`` → no mirror → sequence runs both legs; no advisory hard stop.
- **Opt-in:** ``RUN_V6_3_LLM_SMOKE=1`` and ``OPENAI_API_KEY`` on launch subprocess → ingest,
  mirror, bridge block → first leg advisory → ``stopped_due_to_advisory_blocked`` (provider
  dependent).

Same graph as V5.5 / V6.2; no production code changes.
"""
...
@_SKIP
def test_v63_e2e_real_adapter_missing_key_sequence_no_advisory_hard_stop(
    ros_init, v43_launch_no_llm_secrets
) -> None:
    node = rclpy.create_node("v63_fail_" + str(int(time.time())))
    try:
...
_SKIP_REAL = pytest.mark.skipif(
    ...
    or os.environ.get("RUN_V6_3_LLM_SMOKE", "").strip() != "1",
    reason="Set RUN_V6_3_LLM_SMOKE=1 and OPENAI_API_KEY for opt-in real-model hard-stop smoke.",
)


@_SKIP
@_SKIP_REAL
def test_v63_e2e_real_llm_handoff_sequence_hard_stops_after_advisory_leg(
    ros_init, v43_launch_with_optional_llm_env
) -> None:
    node = rclpy.create_node("v63_ok_" + str(int(time.time())))
    try:
        hcli = node.create_client(
            ProduceSemanticBlockedPassageV35,
            "/produce_semantic_blocked_passage_v35",
        ...
        assert out["steps"][0]["result"].get("navigate_failure_kind") == (
            NAVIGATE_FAILURE_KIND_ADVISORY_BLOCKED_PASSAGE
        )
    finally:
        node.destroy_node()
