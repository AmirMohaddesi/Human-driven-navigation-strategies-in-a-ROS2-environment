...
def _validate_record(
    rec: dict[str, Any], *, allowed_classes: set[str], expected_fact_type: str
) -> bool:
    required_keys = {
        "schema_version",
        "belief_id",
    ...
    speed = rec["recommended_speed_factor"]
    if isinstance(speed, bool) or not isinstance(speed, (int, float)):
        return False
    if float(speed) <= 0.0 or float(speed) > 1.0:
        return False

    dclass = rec["degradation_class"]
    if not isinstance(dclass, str) or dclass not in allowed_classes:
        return False
...
def test_v81_fixture_header_is_frozen() -> None:
    fixture = _load_fixture()
    assert fixture["schema_version"] == "v8.1.fixture.1"
    assert fixture["fact_schema_version"] == "v8.1"
    assert fixture["fact_type"] == "degraded_passage"


def test_v81_fixture_cases_match_expectations() -> None:
    fixture = _load_fixture()
    allowed = set(fixture["allowed_degradation_class"])
